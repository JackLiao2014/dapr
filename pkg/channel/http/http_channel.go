...
var (
	log = logger.NewLogger("dapr.channel.http")
)
...
func (h *Channel) invokeMethodV1(ctx context.Context, req *invokev1.InvokeMethodRequest) (*invokev1.InvokeMethodResponse, error) {
	...
	log.Info("troubleshooting dapr log")
	// change URI to local
	log.Info("before update uri")
	log.Infof("url: %s", channelReq.URI())
	// channelReq.URI().Update("http://127.0.0.1:3502/v1.0/invoke/product-service/method/products")
	log.Info("after update uri")
	log.Infof("url: %s", channelReq.URI())
...
// InvokeMethod invokes user code via HTTP.
func (h *Channel) InvokeMethodViaSidecar(ctx context.Context, req *invokev1.InvokeMethodRequest, appId string, httpPort int) (*invokev1.InvokeMethodResponse, error) {
	// Check if HTTP Extension is given. Otherwise, it will return error.
	httpExt := req.Message().GetHttpExtension()
	if httpExt == nil {
		return nil, status.Error(codes.InvalidArgument, "missing HTTP extension field")
	...
	switch req.APIVersion() {
	case internalv1pb.APIVersion_V1:
		rsp, err = h.invokeMethodV1ViaSidecar(ctx, req, appId, httpPort)
...
func (h *Channel) invokeMethodV1ViaSidecar(ctx context.Context, req *invokev1.InvokeMethodRequest, appId string, httpPort int) (*invokev1.InvokeMethodResponse, error) {
	channelReq := h.constructViaSidecarRequest(ctx, req, appId, httpPort)
...
func (h *Channel) constructViaSidecarRequest(ctx context.Context, req *invokev1.InvokeMethodRequest, appId string, httpPort int) *fasthttp.Request {
	...
	u, _ := url.Parse(h.baseAddress)
	log.Infof("url: %s", u)
	host, _, _ := net.SplitHostPort(u.Host)
	log.Infof("host: %s", host)
	// sample: http://127.0.0.1:3502/v1.0/invoke/product-service/method/products
	// http://127.0.0.1:3502/v1.0/invoke/product-service/method/products
	// http://127.0.0.1:3502/v1.0/invoke/products
	if strings.HasPrefix(method, "/") {
		uri = fmt.Sprintf("%s://%s:%d/v1.0/invoke/%s/method%s", u.Scheme, host, httpPort, appId, method)
	} else {
		uri = fmt.Sprintf("%s://%s:%d/v1.0/invoke/%s/method/%s", u.Scheme, host, httpPort, appId, method)
	}

	log.Infof("sidecar http uri: %s", uri)
